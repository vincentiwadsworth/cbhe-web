 ...
 *
 * When `codigo` is omitted, the database trigger trg_set_certificate_code
 * auto-generates one and returns it (Prefer: return=representation).
 */
async function insertCertificate({ codigo, empresa_nombre, tipo_certificacion, fecha_emision, fecha_vencimiento }) {
  ...
  const body = {
    // codigo omitted when null/undefined so the DB trigger auto-generates it
    ...(codigo ? { codigo } : {}),
    empresa_nombre,
    tipo_certificacion,
    fecha_emision,
...
async function main() {
  ...
  // Use explicit codigo if provided; otherwise let the database auto-generate it
  // via the trg_set_certificate_code trigger on INSERT. The UNIQUE constraint
  // catches explicit-code collisions (no client-side retry needed).
  const codigoExplicito = args["codigo"] || null;

  // Step 1: Insert into Supabase (codigo auto-generated by trigger when omitted)
  ...
  console.log(`  ✓ Inserted (id: ${row.id}, code: ${codigo})`);
